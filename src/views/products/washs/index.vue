<template>
	<Table ref="tableWashsRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader" @selectionChange="selectionChange">
		<template v-slot:command>
			...
			<el-button type="warning" size="small" icon="ele-Upload" round @click="downTemp" v-auth="'products:washs:import'"> 导入产品 </el-button>
		</template>
		...
		<template #action="scope" v-if="auth('products:washs:update') || auth('products:washs:delete')">
			<el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProductWashs(scope.row)" v-auth="'products:washs:update'"> 编辑 </el-button>
			<el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductWashs(scope.row)" v-auth="'products:washs:delete'"> 删除 </el-button>
		</template>
	</Table>
	<editDialog ref="editDialogRef" :productCategorysData="props.productCategorysData" :title="editProductWashsTitle" @reloadTable="handleQuery" />
...
const downTemp = async () => {
	...
	downloadTempRef.value?.openDialog(ProductTypeEnums.NUMBER_40001, [], []);
};
...
const batchDelete = () => {
	...
	ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
		...
		.then(async () => {
			await getAPI(ProductWashsApi)
				.apiProductWashsDeleteByIdsDelete({
					ids: tableRows.map((m) => {
						return m.id;
					}),
				})
