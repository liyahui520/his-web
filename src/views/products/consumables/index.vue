<template>
    <Table ref="tableRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        ...
        <template #action="scope" v-if="auth('products:consumables:update') || auth('products:consumables:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary" v-auth="'products:consumables:update'" @click="openEditProductconsumables(scope.row)"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" v-auth="'products:consumables:delete'" @click="delProductconsumables(scope.row)"> 删除
            </el-button>
        </template>
    </Table>
    <editDialog ref="editDialogRef" :title="editProductconsumablesTitle" @reloadTable="handleQuery"
...
const batchDelete=()=>{
        ...
        ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
            ...
            .then(async () => {
                await getAPI(ProductConsumablesApi).apiProductConsumablesDeleteByIdsDelete({ids: tableRows.map(m=>{return m.id;})}).then(_ => {
                    nextTick(() => {
                        tableRef?.value?.pageReset();
                    });
                    ElMessage.success('删除成功');
