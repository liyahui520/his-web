    ...
    <div class="pcuAccountDetail">
        <el-dialog v-model="isShowDialog" width="70%" draggable :close-on-click-modal="false" destroy-on-close>
          ...
          PcuDetails
            ...
            <Table ref="tableRef" v-bind="tb.tableData" style="height: 550px" :getData="getData" border @sortHeader="onSortHeader"
                   :row-class-name="tableRowClassName">
                <template v-slot:command>
                    <el-row>
                        <el-text tag="b">
                            账户余额：{{ verifyNumberComma(sing?.amountPrice?.toFixed(2).toString() || '0.00') }}
                        </el-text>
                        <el-button type="primary" style="margin-left: 10px" plain size="small" @click="rechargeAccount">
                            充值
                        </el-button>
                ...
                <template v-slot:columncommand>
                    <el-table-column label="支付方式">
                        <el-table-column prop="pcuAccountPayMethods" :label="item.value" :show-overflow-tooltip="true"
                                         min-width="50" v-for="item in getPriceModth">
                            <template #default="scope">
                                {{
                                    (scope.row.tradeType == 1 || scope.row.tradeType == 2) ?
                                            verifyNumberComma(scope.row.pcuAccountPayMethods.filter(s => s.typeId == item.id)[0]?.price?.toFixed(2).toString() || '0.00', true)
                                            :
                                            verifyNumberComma(scope.row.pcuAccountPayMethods.filter(s => s.typeId == item.id)[0]?.price?.toFixed(2).toString() || '0.00')
                                }}
                ...
                <template #action="scope">
                    <el-button icon="ele-Printer" v-if="scope.row.tradeType!=1" size="small" text="" type="primary"
                    > 打印
                    </el-button>
                    <el-button icon="ele-Tickets" v-if="scope.row.tradeType==1" size="small" text="" type="primary"
                    > 详情
                    </el-button>
                </template>
            </Table>
...
<script lang="ts" setup name="pcuAccountDetail">
    ...
    const openDialog = async (customerId) => {
      getPriceModth.value = await dl('funding_method');
