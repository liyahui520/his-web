			...
			<el-form ref="queryForm" :inline="true" label-width="70px">
				...
				<el-form-item label="类型">
					<el-radio-group v-model="checkType">
						...
						<el-radio border :value="ProductTypeEnums.NUMBER_110001">化验</el-radio>
						<el-radio border :value="ProductTypeEnums.NUMBER_150001">试纸</el-radio>
					</el-radio-group>
				</el-form-item>
				<el-form-item>
					<el-button-group>
						<el-button type="primary" icon="ele-Search" @click="handleQuery" v-auth="'laboratorys:tests:query'"> 查询 </el-button>
					</el-button-group>
				</el-form-item>
			</el-form>
		</el-card>
		<el-card class="full-table" shadow="hover" style="margin-top: 8px">
			<Table ref="tableRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader">
				...
				<template #action="scope">
					<el-button v-auth="'laboratorys:tests:edit'" icon="ele-Edit" size="small" text="" type="success" @click="openEditReportResult(scope.row)"> 填写结果 </el-button>
				</template>
			</Table>
		</el-card>
		<editResport ref="editResportRef" :title="editResportTitle" @reloadTable="handleQuery" />
	</div>
</template>
...
const editResportRef = ref<RefType>();
...
const editResportTitle = ref<string>('');
const getData = async (par: any) => {
	const params = { startDate: dateValue.value[0], endDate: dateValue.value[1], type: checkType.value };
	let res = await getAPI(LaboratoryApi).apiLaboratoryGetReportTestPagePost(Object.assign(params, par));
	return res.data;
};
...
const openEditReportResult = (row: any) => {
	editResportTitle.value = '编辑报告结果';
	editResportRef.value.openDialog({ ...row, isEdit: true });
};
