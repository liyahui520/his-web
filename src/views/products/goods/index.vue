    ...
    <Table ref="tableRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        ...
        <template v-slot:command>
            <el-button type="danger" :icon="Delete" @click="batchDelete" plain size="small"
                v-auth="'products:goods:delete'">批量删除</el-button>
            <el-button type="primary" :icon="Edit" plain size="small" @click="batchEdit"
                v-auth="'products:goods:update'">批量编辑</el-button>
        </template>
        ...
        <template #action="scope" v-if="auth('products:goods:update') || auth('products:goods:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProductgoods(scope.row)"
                v-auth="'products:goods:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductgoods(scope.row)"
                v-auth="'products:goods:delete'"> 删除
            </el-button>
        </template>
    </Table>
...
const batchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
        ...
        .then(async () => {
            await getAPI(ProductGoodsApi).apiProductGoodsDeleteByIdsDelete({ ids: tableRows.map(m => { return m.id; }) }).then(_ => {
                nextTick(() => {
                    tableRef?.value?.pageReset();
                });
...
const getData = async (par: any) => {
    let res = await getAPI(ProductGoodsApi).apiProductGoodsPagePost(par);
    return res.data;
}
...
const openEditProductgoods = (row: any) => {
    ...
    row.categoryArr = row.categoryId == -1 ? [] : ['1202000000001', row.categoryId]
