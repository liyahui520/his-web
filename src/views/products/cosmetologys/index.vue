<template>
    <Table ref="tableCosmetoloysRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        ...
        <template #action="scope" v-if="auth('products:cosmetologys:update') || auth('products:cosmetologys:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary"
                       @click="openEditProductCosmetologys(scope.row)"
                       v-auth="'products:cosmetologys:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductCosmetologys(scope.row)"
                       v-auth="'products:cosmetologys:delete'"> 删除
            </el-button>
        </template>
    </Table>

    <editDialog ref="editDialogRef" :title="editProductCosmetologysTitle" :productCategorysData="props.productCategorysData"
...
import batchEditDialog from '/@/views/products/component/batchEditDialog.vue';
    ...
    const tb = reactive<TableDemoState>({
        tableData: {
            ...
            config: {
                ...
                exportFileName: '美容信息', //导出报表的文件名，不填写取应用名称
                isDisabled:false,
                hidePrint: true
            },
    ...
    const handleQuery = async (data: any) => {
        ...
        await nextTick(() => {
            tableCosmetoloysRef?.value?.pageReset();
        });
    };
    const getData = async (par: any) => {
        let res = await getAPI(ProductCosmetologysApi).apiProductCosmetologysPagePost(par);
        return res.data;
    }
    //选中的值
...
//批量编辑
const batchEdit = () => {
    const tableRows = tableCosmetoloysRef.value.getSelectionRows();
    ...
    const batchDelete=()=>{
        ...
        ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
            ...
            .then(async () => {
                await getAPI(ProductCosmetologysApi).apiProductCosmetologysDeleteByIdsDelete({ids: tableRows.map(m=>{return m.id;})}).then(_ => {
                    nextTick(() => {
                        tableCosmetoloysRef?.value?.pageReset();
                    });
