<template>
    <Table ref="tableDrugsRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        ...
        <template v-slot:command>
            <el-button type="danger" :icon="Delete" @click="batchDelete" plain size="small"
                v-auth="'products:drugs:delete'">批量删除</el-button>
            <el-button type="primary" :icon="Edit" plain size="small" @click="batchEdit"
                v-auth="'products:drugs:update'">批量编辑</el-button>
            <!-- <el-button type="info" :icon="Flag" plain size="small">批量标识</el-button>
            <el-button type="warning" :icon="Failed" plain size="small">清空标识</el-button> -->
        </template>
        ...
        <template #action="scope" v-if="auth('products:drugs:update') || auth('products:drugs:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProductDrugs(scope.row)"
                v-auth="'products:drugs:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductDrugs(scope.row)"
                v-auth="'products:drugs:delete'"> 删除
            </el-button>
        </template>
    </Table>
    <editDialog ref="editDialogRef" :productCategorysData="props.productCategorysData" :title="editProductDrugsTitle"
...
const downTemp = async () => {
    var r = await getAPI(ProductDrugsApi).apiProductDrugsDownGet({ headers: { responseType: 'blob' } });
    //downloadByData(r.data.result,'药品信息模板'+formatDate(new Date(),'YYYYmmddHHMMSS')+'.xlsx')
    //  console.log('下载的文件',blobToFile(r,'药品信息模板'+formatDate(new Date(),'YYYYmmddHHMMSS')+'.xlsx','xlsx'))
    await downloadContent(r, '药品信息模板' + formatDate(new Date(), 'YYYYmmddHHMMSS') + '.xlsx')
}

const downloadContent = async (value, name) => {
    ...
    downloadLink.setAttribute('download', name); // 设置文件名
...
const batchDelete = async () => {
    ...
    ElMessageBox.confirm(`确定要删除吗?共` + deleteIds.value.length + `条数据`, '提示', {
        ...
        .then(async () => {
            await getAPI(ProductDrugsApi).apiProductDrugsDeleteByIdsDelete({ ids: deleteIds.value }).then(_ => {
                nextTick(() => {
                    tableDrugsRef?.value?.pageReset();
                });
...
const openAddProductDrugs = (data: any) => {
    ...
    editDialogRef.value?.openDialog({
        ...
        memberPrice: 0,
        inSpecific: 1
    },usingMethodData.value,dosingWayData.value);
};
