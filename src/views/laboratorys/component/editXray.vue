				...
				<div style="padding-left: 20px; border-top: 1px solid var(--el-card-border-color); padding-top: 20px; padding-bottom: 20px; padding-right: 5px">
					<el-form v-loading="drawerLoading" :model="saveXrayInspectForm" label-width="auto" label-position="left" size="large">
							...
							<div class="report-image" v-for="item in saveXrayInspectForm.xrayInspectImgList" :key="item.id" v-show="item.isDelete === false">
								<div v-if="item.isDelete === false">
									...
									<div class="img-conver">
										...
										<a style="margin-left: 10px; padding: 0px 4px; cursor: pointer; text-decoration: none; color: #fff" :href="item.xrayImageUrl" target="_blank">打开</a>
									</div>
									<div class="report-info">
										<div>
											<el-input v-model="item.xrayImagePartName" style="width: 100%" :rows="2" placeholder="请填写图像部位">
												<template #prepend>部位/描述</template>
											</el-input>
										</div>
										<div style="margin-top: 4px">
											<el-input v-model="item.xrayImageDescribe" type="textarea" style="width: 100%" :rows="3" placeholder="请填写图像描述">
						...
						<el-form-item label="检查医生：">
							<div>
								<el-select v-model="saveXrayInspectForm.checkDoctorId" placeholder="请选择检查医生" style="width: 200px">
						...
						<el-form-item label="报告医生：">
							<div>
								<el-select v-model="saveXrayInspectForm.reportDoctorId" placeholder="请选择报告医生" style="width: 200px">
									<el-option v-for="item in doctorUserData" :key="item.id" :label="item.realName" :value="item.id" />
								</el-select>
								<span style="padding: 10px"> | </span>
						...
						<el-form-item label="助理：">
							<div>
								<el-select v-model="saveXrayInspectForm.assistantId" placeholder="请选择助理" style="width: 200px">
									<el-option v-for="item in doctorUserData" :key="item.id" :label="item.realName" :value="item.id" />
								</el-select>
							</div>
						</el-form-item>
...
import { XrayInspectApi } from '/@/api-services/api';
import { SysFileApi } from '/@/api-services/api';
import { SaveXrayInspectInput } from '/@/api-services/models/laboratory-manage/save-xray-inspect-input';
import { SaveXrayInspectImgInput } from '/@/api-services/models/laboratory-manage/save-xray-inspect-img-input';
...
const getXrayInspectResult = async (cEMRecordTestItemId: number) => {
	...
	saveXrayInspectForm.cemRecordTestItemId = cEMRecordTestItemId;

	var res = await getAPI(XrayInspectApi).apiXrayInspectByRecordTestItemIdCEMRecordTestItemIdGet(cEMRecordTestItemId);
	if (res.data.result) {
		var result = res.data.result;
		saveXrayInspectForm.xrayDescribe = result.xrayDescribe;
		saveXrayInspectForm.xrayTips = result.xrayTips;
		saveXrayInspectForm.xrayPropose = result.xrayPropose;
		...
		result.xrayInspectImgList?.forEach((element) => {
			previewImage.imgs.push(element.xrayImageUrl);
		});
	} else {
		saveXrayInspectForm.xrayDescribe = null;
...
const save = async () => {
	...
	saveXrayInspectForm.checkDoctorName = doctorUserObject.value[saveXrayInspectForm.checkDoctorId ?? 0];
	saveXrayInspectForm.reportDoctorName = doctorUserObject.value[saveXrayInspectForm.reportDoctorId ?? 0];
	saveXrayInspectForm.assistantName = doctorUserObject.value[saveXrayInspectForm.assistantId ?? 0];
	await getAPI(XrayInspectApi)
		.apiXrayInspectSavePost(saveXrayInspectForm)
		.then((res) => {
...
// 删除图片
const deleteImage = (img: SaveXrayInspectImgInput) => {
	img.isDelete = true;
	previewImage.imgs = previewImage.imgs.filter((item) => item != img.xrayImageUrl);
};
// 上传图片处理
const uploadPetImageUrlHandle = async (options: any) => {
	...
	var result = res.data.result;
	if (result) {
		const img: SaveXrayInspectImgInput = {
			id: 0,
			xrayImageUrl: result.url!,
			xrayImageDescribe: '',
			xrayImagePartName: '',
			isDelete: false,
		};
		saveXrayInspectForm.xrayInspectImgList?.push(img);
