<template>
	<div class="product-vaccines-container">
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id != undefined && ruleForm.value.id > 0) {
				await getAPI(ProductVaccinesApi).apiProductVaccinesEditPut(values);
			} else {
				await getAPI(ProductVaccinesApi).apiProductVaccinesAddPost(values);
			}
			closeDialog();
		}
	});
