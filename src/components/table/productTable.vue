<template>
    <div class="table-container" style="width: 100%">
        <div class="table-header mb5"  v-if="!hideTool">
            <div>
                <slot name="command"></slot>
            </div>
            <div v-loading="state.importLoading" class="table-footer-tool">
                <SvgIcon name="iconfont icon-shuaxin" v-if="!config.hideLoad" :size="22" title="刷新"
                    @click="onRefreshTable" />
                <el-icon @click="onPrintTable" v-if="!config.hidePrint" :size="16" title="打印">
                    <Printer />
                </el-icon>
                ...
                <el-popover placement="top-end" v-if="!config.hideSort" trigger="click" transition="el-zoom-in-top"
        ...
        <el-table ref="tableRef" :data="state.data" :border="setBorder" :stripe="setStripe" v-bind="$attrs" row-key="id" 
            default-expand-all  v-loading="state.loading" :default-sort="defaultSort"
            @selection-change="onSelectionChange" @sort-change="sortChange" :row-class-name="props.rowClassName">
            <slot name="columncommand"></slot>
            <el-table-column type="selection" :reserve-selection="true" :width="40" align="center"
                v-if="config.isSelection && config.showSelection" />
            <el-table-column type="index" label="序号" align="center" :width="60" v-if="config.isSerialNo" fixed="left" >
        ...
        <div v-if="state.showPagination && !props.config.showPagination" class="table-footer">
            <el-pagination v-model:current-page="state.page.page" v-model:page-size="state.page.pageSize"
...
import { reactive, computed, nextTick, ref, onMounted, defineAsyncComponent } from 'vue';
...
const onSelectionChange = (val: EmptyObjectType[]) => {
    ...
    // console.log("tableRef。value::::",tableRef.value.getSelectionRows())
...
const importData = (data: Array<EmptyObjectType>) => {
    ...
    state.importLoading = true;
    let exportData = other.deepClone(data) as any;
...
const onPrintTable = () => {
    ...
    let tableTd: any = {};
    // 表头
    props.columns.filter((v) => v.isCheck).forEach((v: any) => {
        if (v.type != 'action')
    ...
    state.data.forEach((val: any, key: any) => {
        ...
        props.columns.filter((v) => v.isCheck).forEach((v: any) => {
            if (v.type === 'text') {
                ...
                tableTd[key].push(`<td class="table-th table-center">${verifyNumberComma(val[v.prop]?.toFixed(2)) || '0.00'}</td>`);
...
const getSelectionRows = (row: any, statu?: boolean) => {
