<template>
    <Table ref="tableBModesRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        ...
        <template #action="scope" v-if="auth('products:bmodes:update') || auth('products:bmodes:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProductBModes(scope.row)" v-auth="'products:bmodes:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductBModes(scope.row)" v-auth="'products:bmodes:delete'"> 删除
            </el-button>
        </template>
    </Table>
...
const batchDelete=()=>{
        ...
        ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
            ...
            .then(async () => {
                await getAPI(ProductBModeApi).apiProductBModeDeleteByIdsDelete({ids: tableRows.map(m=>{return m.id;})}).then(_ => {
                    nextTick(() => {
