				...
				<el-row>
					<el-col :span="24">
						<el-form-item prop="typeId" style="display: flex">
							...
							<el-select v-model="ruleForm.typeId" placeholder="请选择押金类型" size="default" style="width: 240px">
								<el-option v-for="item in props.depositTypes" :key="item.id" :label="item.name" :value="item.id" />
							</el-select>
						</el-form-item>
					</el-col>
					<el-col style="display: flex" :span="12" v-for="(item, index) in ruleForm.payMethods" :key="index">
						<el-form-item :prop="'methodId' + item.methodId">
							<div style="display: flex; flex-direction: row; justify-content: center">
								...
								<span style="margin-left: 10px; width: 50px">{{ getPriceModthObject[item.methodId!].name }}</span>
							</div>
						</el-form-item>
...
const submitForm = async () => {
	...
	await getAPI(MemberDepositApi)
		.apiMemberDepositRechargeDepositPost(ruleForm.value)
		.then((res) => {
