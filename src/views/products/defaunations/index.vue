<template>
    <Table ref="tableDefaunationsRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        @selectionChange="selectionChange">
        <template v-slot:command>
            <el-button type="danger" :icon="Delete" plain size="small" @click="batchDelete"
                v-auth="'products:defaunations:delete'">批量删除</el-button>
            <el-button type="primary" :icon="Edit" plain size="small" @click="batchEdit"
                v-auth="'products:defaunations:update'">批量编辑</el-button>
        </template>
        ...
        <template #action="scope" v-if="auth('products:defaunations:update') || auth('products:defaunations:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary"
                @click="openEditProductDefaunations(scope.row)" v-auth="'products:defaunations:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductDefaunations(scope.row)"
                v-auth="'products:defaunations:delete'"> 删除
            </el-button>
        </template>
    </Table>
    <editDialog ref="editDialogRef" :title="editProductDefaunationsTitle" @reloadTable="handleQuery"
...
const batchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
        ...
        .then(async () => {
            await getAPI(ProductDefaunationsApi).apiProductDefaunationsDeleteByIdsDelete({ ids: tableRows.map(m => { return m.id; }) }).then(_ => {
                nextTick(() => {
                    tableDefaunationsRef?.value?.pageReset();
                });
