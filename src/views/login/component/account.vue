<template>
	<el-tooltip :visible="state.capsLockVisible" effect="light" content="大写锁定已打开" placement="top">
		<el-form ref="ruleFormRef" :model="state.ruleForm" size="large" :rules="state.rules" class="login-content-form">
			...
			<el-form-item class="login-animation4" prop="isCloud">
				<el-checkbox ref="isCloudRef" label="同意" v-model="state.ruleForm.isCloud" /><el-link type="primary" style="line-height: 14px;">《用户协议》</el-link>和<el-link type="primary" style="line-height: 14px;">《隐私协议》</el-link>
...
const isCloudRef = ref<InputInstance>();
...
const state = reactive({
	...
	rules: {
		...
		isCloud: [{ required: true, message: '请先阅读《用户协议》和《隐私协议》', trigger: 'change' }],
...
const receiveNotice = (msg: any) => {
  ...
  try {
    ...
    saveTokenAndInitRoutes(msg?.accessToken);
...
const onSignIn = async () => {
	ruleFormRef.value.validate(async (valid: boolean) => {
		...
		if(!state.ruleForm.isCloud){
			ElMessage.error('请先阅读《用户协议》和《隐私协议》！');
			return false;
		}
		try {
			state.loading.signIn = true;
