		...
		<el-card shadow="never" style="overflow: auto">
			...
			<span>
				...
				<el-text class="w-150px mb-2" style="margin-left: 20px; font-size: 12px; color: #666666; line-height: 30px" truncated
					>先启动本地<el-button key="primary" type="primary" link>打印机服务</el-button>
					，然后在服务中添加远程打印机。
				</el-text>
					...
					<el-form :model="serverform" label-width="auto">
						...
						<el-form-item label="病例打印机">
							<el-select placeholder="请选择病例打印机" style="width: 240px">
								<el-option v-for="item in printDevies" :key="item.name" :label="item.name" :value="item.name" />
							</el-select>
							<el-input v-model="serverform.pageCount" style="max-width: 100px">
