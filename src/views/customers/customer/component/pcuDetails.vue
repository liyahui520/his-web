				...
				<el-card shadow="never" style="overflow: auto" :body-style="{ padding: 0 }" v-loading="petLoading">
					<el-scrollbar height="calc(100vh - 225px)">
						...
						<span style="float: left; margin: 10px; width: 380px" :key="item.id" v-for="item in getPPets">
							<CardPet :pet="item" @petsReload="initData({ id: customerValue.id })" />
						</span>
					</el-scrollbar>
				</el-card>
...
import CardPet from '/@/views/customers/pet/component/cardPet.vue';
import CardAdd from '/@/views/customers/pet/component/cardAdd.vue';
...
const customerValue = ref<any>({});
const initPcustomer = async () => {
	await initData({ id: customerValue.value.id });
};
