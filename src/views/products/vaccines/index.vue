<template>
    <Table ref="tableVaccinesRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        @selectionChange="selectionChange">
        <template v-slot:command>
            <el-button type="danger" :icon="Delete" plain size="small" @click="batchDelete"
                v-auth="'products:vaccines:delete'">批量删除</el-button>
            <el-button type="primary" :icon="Edit" plain size="small" @click="batchEdit"
                v-auth="'products:vaccines:update'">批量编辑</el-button>
        </template>
        ...
        <template #action="scope" v-if="auth('products:vaccines:update') || auth('products:vaccines:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProductVaccines(scope.row)"
                v-auth="'products:vaccines:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductVaccines(scope.row)"
                v-auth="'products:vaccines:delete'"> 删除
            </el-button>
        </template>
    </Table>
    <editDialog ref="editDialogRef" :title="editProductVaccinesTitle" @reloadTable="handleQuery"
...
const intervalUnitObject = { 'Day': '天', 'Week': '周', 'Month': '月', 'Year': '年' }
...
const batchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
        ...
        .then(async () => {
            await getAPI(ProductVaccinesApi).apiProductVaccinesDeleteByIdsDelete({ ids: tableRows.map(m => { return m.id; }) }).then(_ => {
                nextTick(() => {
                    tableVaccinesRef?.value?.pageReset();
                });
                ElMessage.success('删除成功');
