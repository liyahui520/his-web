  ...
  <div class="table-container">
    <div class="table-header mb15">
      ...
      <div v-loading="state.importLoading" class="table-footer-tool">
        <SvgIcon name="iconfont icon-shuaxin" v-if="!config.hideLoad" :size="22" title="刷新" @click="onRefreshTable"/>
        ...
        <el-popover placement="bottom-end" v-if="!config.hideSort" trigger="click" transition="el-zoom-in-top"
            ...
            <el-scrollbar>
              <div ref="toolSetRef" class="tool-sortable">
                <div class="tool-sortable-item" v-for="v in columns" :key="v.prop"
                  ...
                  <el-checkbox :checked="!v.hidden" size="default" class="ml12 mr8" :label="v.label"
                               @change="onCheckChange(v)"/>
                </div>
              </div>
            </el-scrollbar>
          </template>
    ...
    <el-auto-resizer>
      <template #default="{ height, width }">
        <el-table-v2 ref="tableRef" :columns="props.columns" :data="state.data" :border="setBorder" :stripe="setStripe"
                     v-bind="$attrs" row-key="id" :fixed="true"
                     default-expand-all v-loading="state.loading" :default-sort="defaultSort" expand-column-key="id"
                     :height="height" :width="width"
                     @selection-change="onSelectionChange" @column-sort="sortChange">

               <!-- <el-table-column type="selection" :reserve-selection="true" :width="30"
                                v-if="config.isSelection && config.showSelection" />
               <el-table-column type="index" label="序号" align="center" fixed="left" :width="60" v-if="config.isSerialNo" /> -->
               
          <!--      <el-table-column v-for="(item, index) in setHeader" :key="index" v-bind="item">-->
          <!--        &lt;!&ndash; 自定义列插槽，插槽名为columns属性的prop &ndash;&gt;-->
...
<!--                  </template>-->
    ...
    <div v-if="state.showPagination&&!props.config.showPagination" class="table-footer mt15">
...
const onCheckAllChange = <T>(val: T) => {
  if (val) props.columns.forEach((v) => (v.hidden = false));
  else props.columns.forEach((v) => (v.hidden = true));
...
const onCheckChange = (v) => {
  ...
  const headers = props.columns.filter((v) => !v.hidden).length;
