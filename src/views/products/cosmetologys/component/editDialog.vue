<template>
    <div class="product-cosmetologys-container">
...
const ruleForm = ref<any>({ isAnaesthesia: false });
...
const rules = ref<FormRules>({
    ...
    outUnitId: [{ required: true, message: '请选择单位！', trigger: 'change' }],
});
...
const openDialog = (row: any) => {
    ...
    if(!unitObject.value[ruleForm.value.outUnitId]){
        ruleForm.value.outUnitId=null;
    }
    ...
    ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
        if (isValid) {
            ...
            if (ruleForm.value.id != undefined && ruleForm.value.id > 0) {
                await getAPI(ProductCosmetologysApi).apiProductCosmetologysEditPut(values);
            } else {
                await getAPI(ProductCosmetologysApi).apiProductCosmetologysAddPost(values);
            }
            resetForm(ruleFormRef);
            closeDialog();
        }
...
.product-cosmetologys-container {
    .el-tag {
        white-space: normal;
        height: auto;
        padding: 9px 9px;
