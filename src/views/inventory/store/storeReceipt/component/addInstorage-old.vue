		...
		<el-dialog v-model="isShowDialog" ref="instorageForm" draggable :close-on-click-modal="false" :fullscreen="true" :destroy-on-close="true" @closed="dialogClosed">
			...
			<template #footer>
				<span style="float: left">
					...
					<span style="float: left; margin-left: 50px">
						<p style="font-size: 18px; font-weight: bold">总量：{{ queryParams.inCount }} 项</p>
					</span>
				</span>
...
<script lang="ts" setup name="instorage-old">
...
// const StampBadge =  defineAsyncComponent(() => import('/@/components/StampBadge/src/StampBadge.vue'));
...
const queryParams = ref<any>({
	...
	amount: 0.0,
	inCount: 0,
	orderNo: 'R' + formatDate(new Date(), 'YYYYmmddHHMMSS'),
	differCount: 0,
	differAmount: 0.0,
});
...
const state = reactive<any>({
	...
	tableData: {
		...
		header: [
			{ prop: 'productCode', minWidth: '80', label: '编号' },
			{ prop: 'categoryName', minWidth: '80', label: '目录' },
			{ prop: 'productTypeName', minWidth: '60', label: '项目类别' },
			{ prop: 'productName', minWidth: '150', label: '产品名称', type: 'dialog', placeholder: '查询产品' },
			{ prop: 'specific', minWidth: '80', label: '规格' },
			{ prop: 'unitName', minWidth: '80', label: '单位' },
			{ prop: 'procureCount', minWidth: '180', label: '采购数', isRequired: true, type: 'number', min: 1 },
			{ prop: 'giveCount', minWidth: '180', label: '赠品数', type: 'number', min: 0 },
			{ prop: 'amount', minWidth: '180', label: '总金额', type: 'number', precision: 2 },
			{
				prop: 'inPrice',
				width: '',
				label: '采购单价',
			...
			{ prop: 'remark', width: '180', label: '备注', type: 'input', placeholder: '请输入备注' },
			{
				prop: 'action',
				width: 100,
				label: '操作',
...
const remove = async (row: any) => {
	...
	queryParams.value.amount = state.tableData.data.reduce((total, object) => total + object.amount, 0.0);
	queryParams.value.inCount = state.tableData.data.length - 1;
};
	...
	data.forEach((v) => {
		...
		if (!found)
			state.tableData.data.push({
				...
				amount: v.amount,
				inPrice: v?.inPrice ?? 0.0,
				singPrice: v.singPrice, // v.singPrice,
				remark: v.remark,
				ver: v.ver,
			});
	});
	if (!state.isDisabled) queryParams.value.inCount = state.tableData.data.length - 1;
};
...
const openDialog = async (isEdit: boolean, row: any, isDisabled: boolean = false, isHeddin: boolean = false) => {
	...
	if (isEdit) {
		...
		await importEditProduct(row.inStorageDetails);
	} else {
...
const submit = () => {
	queryForm.value.validate(async (valid: boolean) => {
		...
		queryParams.value.inStorageDetails = rows;
