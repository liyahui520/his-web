<template>
	<el-card shadow="never" header="近 7 日收入" v-loading="loading">
...
export default {
	...
	methods: {
		async getData() {
			let _this = this;
			_this.loading = true;
			await getAPI(OrderSummaryApi).apiOrderSummaryLastFittenPayGet().then((res) => {
				...
				result.forEach((item) => {
					...
					_this.option.series[0].data.push({
						name: item.date,
						value: new Decimal(item.totalAmount).toNumber().toFixed(2),
						extend: `${item.date}<br/>
						${markerTotal} 总收款金额：${new Decimal(item.amount).toNumber().toFixed(2)}<br/>
						${markerReturn} 总退款金额：${new Decimal(item.returnAmount).toNumber().toFixed(2)}<br/>
