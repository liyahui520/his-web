<template>
    <div class="product-xRays-container">
    ...
    ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
        if (isValid) {
            ...
            if (ruleForm.value.id != undefined && ruleForm.value.id > 0) {
                await getAPI(ProductXRayApi).apiProductXRayEditPost(values);
            } else {
                await getAPI(ProductXRayApi).apiProductXRayAddPost(values);
            }
            closeDialog();
        }
    });
