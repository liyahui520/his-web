<template>
	<div class="product-fosters-container">
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id != undefined && ruleForm.value.id > 0) {
				await getAPI(ProductFostersApi).apiProductFostersEditPut(values);
			} else {
				await getAPI(ProductFostersApi).apiProductFostersAddPost(values);
			}
			closeDialog();
		}
	});
