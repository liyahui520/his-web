<template>
    <Table ref="tableFostersRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        @selectionChange="selectionChange">
        <template v-slot:command>
            <el-button type="danger" :icon="Delete" plain size="small" @click="batchDelete"
                v-auth="'products:fosters:delete'">批量删除</el-button>
            <el-button type="primary" :icon="Edit" plain size="small" @click="batchEdit"
                v-auth="'products:fosters:update'">批量编辑</el-button>
        </template>
        ...
        <template #action="scope" v-if="auth('products:fosters:update') || auth('products:fosters:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProductFosters(scope.row)"
                v-auth="'products:fosters:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductFosters(scope.row)"
                v-auth="'products:fosters:delete'"> 删除
            </el-button>
        </template>
    </Table>

    <editDialog ref="editDialogRef" :title="editProductFostersTitle" @reloadTable="handleQuery"
...
import editDialog from '/@/views/products/fosters/component/editDialog.vue';
import { Delete, Edit, Flag, Failed } from '@element-plus/icons-vue'
import { getAPI } from '/@/utils/axios-utils';
import { ProductFostersApi } from '/@/api-services/api';
...
const tableFostersRef = ref();
const deleteIds = ref<any>([]);
const editProductFostersTitle = ref('');
...
const getData = async (par: any) => {
    let res = await getAPI(ProductFostersApi).apiProductFostersPagePost(par);
    return res.data;
}
...
const handleQuery = async (data: any) => {
    ...
    await nextTick(() => {
        tableFostersRef?.value?.pageReset();
    });
    loading.value = false;
};
...
// 删除
const delProductFosters = (row: any) => {
    ElMessageBox.confirm(`确定要删除吗?`, '提示', {
        ...
        .then(async () => {
            await getAPI(ProductFostersApi).apiProductFostersDeleteDelete(row);
            await nextTick(() => {
                tableFostersRef?.value?.pageReset();
            });
...
//批量编辑
const batchEdit = () => {
    const tableRows = tableFostersRef.value.getSelectionRows();
...
const batchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
        ...
        .then(async () => {
            await getAPI(ProductFostersApi).apiProductFostersDeleteByIdsDelete({ ids: tableRows.map(m => { return m.id; }) }).then(_ => {
                nextTick(() => {
                    tableFostersRef?.value?.pageReset();
                });
                ElMessage.success('删除成功');
...
onMounted(async () => {
    // 监听布局配'置弹窗点击打开
    mittBus.on('fostersQueryMthods', async (params) => {
        await handleQuery(params);
    });
    // 监听布局配'置弹窗点击打开
    mittBus.on('fostersAddMthods', async (params) => {
        openAddProductFosters(params);
    });
});
