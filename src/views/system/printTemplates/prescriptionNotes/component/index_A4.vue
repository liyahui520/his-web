<template>
	<el-row :gutter="4" style="margin: 0px auto; height: 200mm">
		...
		<el-col :span="15">
			<el-card shadow="never" class="card-design full-table" style="width: 220mm; margin: 0 auto; height: 300mm" v-loading="loading">
				<template #header>
...
var props = defineProps({
	printType: {
		type: Number,
		default: PrintTypeEnum.NUMBER_2001,
	},
	Providers: {
		type: Object,
...
const handleQuery = async () => {
	...
	if (res.data.result?.length ?? 0 > 0) {
		...
		InitmodeList(props.tempDatas, props.Providers);
	}
...
const InitmodeList = (datas, p) => {
	...
	state.modeList.push(datas);
	state.enablename = state.modeList.find((s: any) => s.status === 1)?.name;
	...
	console.log("props.Providers.value",props.Providers.f())
	InitPrintTemp();
};

const InitPrintTemp = () => {
	hiprintTemplate.value.clear();
	hiprintTemplate.value.update(JSON.parse(props.Providers.value));
};
