	...
	<div class="sys-user-container">
		<el-dialog v-model="state.isShowDialog" draggable :close-on-click-modal="false" width="700px">
			...
			<el-tabs v-loading="state.loading" v-model="state.selectedTabName">
				...
				<el-tab-pane label="收费管理">
					<el-transfer v-model="state.ruleForm.productIdList" style="text-align: left; display: inline-block" filterable :titles="['可选', '已选']"
						:button-texts="['移除', '添加']" :right-default-checked="state.ruleForm.productIdList" :format="{
							noChecked: '${total}',
							hasChecked: '${checked}/${total}',
						}" :data="chargeData" @change="handleChargeChange">
...
<script lang="ts" setup name="sysEditUser-Old">
import { onMounted, reactive, ref } from 'vue';
import { storeToRefs } from 'pinia';
import { useUserInfo } from '/@/stores/userInfo';

import { getAPI } from '/@/utils/axios-utils';
import { SysPosApi, SysRoleApi, SysUserApi,ProductRegistersApi } from '/@/api-services/api';
...
const loadChargeData=async ()=>{
	var res = await getAPI(ProductRegistersApi).apiProductRegistersGetSelectListPost();
	var result = res.data.result ?? [];
	result.forEach(element => {
		chargeData.value.push({ key:element.id, label:element.name })
	});
}
