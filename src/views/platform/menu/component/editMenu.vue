...
import { SysMenuApi, SysEnumApi } from '/@/api-services/api';
...
const state = reactive({
    isShowDialog: false,
    ruleForm: { classInfo: { dllName: '', classFillName: '' }, largeImage: '', image: '' } as unknown as UpdateMenuInput,
...
// 提交
const submit = () => {
    console.log("state.ruleForm::::", ruleFormRef.value)
    ruleFormRef.value.validate(async (valid: boolean) => {
        if (!valid) return;
        // let d = Object.assign({},state.ruleForm,{classInfo:state.classInfo});
...
const init = async () => {
    let r = await getAPI(SysEnumApi).apiSysEnumEnumDataListGet('FunctionType');
    functionTypes.value = r.data?.result ?? [];
    let rr = await getAPI(SysEnumApi).apiSysEnumEnumDataListGet('FunctionItemType');
    itemTypes.value = rr.data?.result ?? [];
}
