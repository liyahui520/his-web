		...
		<el-dialog v-model="isShowDialog" :title="props.title" :width="'40%'" draggable :close-on-click-modal="false">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" size="default" label-width="100px" :rules="rules">
				...
				<el-divider content-position="left" style="width:98%; margin: 15px">
                    <el-button type="primary" :icon="Plus" @click="addGoodsItems" plain
                            ...
                            <el-table-column label="操作" width="80" align="center" fixed="right">
                                <template #default="scope">
                                    <el-button icon="ele-Delete" size="small" text="" type="danger"
                                        @click="deleteGoodsItems(scope.$index)"> 移除 </el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                    </el-col>
...
const handleSelectProduct = (data: any) => {
    data.forEach((item: any) => {
        if (ruleForm.value.productItems.findIndex((i: any) => i.productId == item.productId) == -1) {
            ruleForm.value.productItems.push({productId:item.productId,productName:item.productName,salePrice:item.salePrice,outUnitId:item.outUnitId,outUnitName:unitObject.value[item.outUnitId],typeText:item.productTypeText,type:item.productType});
        }
    })
}
/**
