<template>
    <div class="product-washs-container">
...
const findParentIds=(tree:any, targetId:any, parentIds = <any>[]): any[] => {
    ...
    ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
        if (isValid) {
            ...
            if (ruleForm.value.id != undefined && ruleForm.value.id > 0) {
                await getAPI(ProductWashsApi).apiProductWashsEditPut(values);
            } else {
                await getAPI(ProductWashsApi).apiProductWashsAddPost(values);
            }
            closeDialog();
        }
    });
