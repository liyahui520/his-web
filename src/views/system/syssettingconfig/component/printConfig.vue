				...
				<el-row :gutter="10" style="padding: 0px 40px 0px">
					<el-form :model="serverform" label-width="auto">
						<el-form-item label="授权码">
							<el-input style="width: 340px" v-model="printConfig.app.machineId" placeholder="请输入授权码" />
...
const btnCon = async () => {
	if (printConfig.value.app.machineId) {
		hiprint.hiwebSocket.getPrintById(printConfig.value.app.machineId);
		hiprint.refreshPrinterList((res: any) => {
			printDevies.value = res;
			getAPI(PrintSettingApi).sernumberNoGetPrintConnectionIdGet(printConfig.value.app.machineId).then((res: any) => {
...
const init = async () => {
	// 获取打印机配置
	await getAPI(PrintSettingApi).getPrintSettingConfigGet().then((res: any) => {
