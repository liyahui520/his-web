	...
	<div class="rechargeCard-container">
		<el-dialog v-model="isShowDialog" width="50%" draggable :close-on-click-modal="false" destroy-on-close>
					...
					<el-col :span="24">
						<el-form-item prop="giveAmount" style="display: flex">
							...
							<el-input-number
								...
								:step="100"
								:max="9999999"
								:disabled="props.cardInfo.giveAmount == 0"
								@change="
									(currentValue: any, oldValue: any) => {
			...
			<el-descriptions style="margin-top: 30px" :column="3" :border="false" size="large" align="center">
				<el-descriptions-item label="可退本金"
					><el-text tag="b">{{ props.cardInfo.oriAmount }}</el-text></el-descriptions-item
				>
				<el-descriptions-item label="可退赠送"
					><el-text tag="b">{{ props.cardInfo.giveAmount }}</el-text></el-descriptions-item
				>
				<el-descriptions-item label="可退金额"
					><el-text tag="b">{{ props.cardInfo.totalAmount }}</el-text></el-descriptions-item
				>
				<el-descriptions-item label="本金退款"
					><el-text tag="b" type="danger">- {{ computedOriAmout }}</el-text></el-descriptions-item
				>
				<el-descriptions-item label="赠送退款"
					><el-text tag="b" type="danger">- {{ ruleForm.giveAmount }}</el-text></el-descriptions-item
				>
				<el-descriptions-item label="总退金款"
					><el-text tag="b" type="danger">- {{ computedTotalAmout }}</el-text></el-descriptions-item
				>
			</el-descriptions>
...
const submitForm = async () => {
	...
	await getAPI(MemberCardApi)
		.apiMemberCardReturnMembercardPost(ruleForm.value)
		.then((res) => {
