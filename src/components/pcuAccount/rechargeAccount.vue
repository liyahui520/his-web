        ...
        <el-dialog v-model="isShowDialog" width="50%" draggable :close-on-click-modal="false" destroy-on-close>
                    ...
                    <el-col :span="24">
                        ...
                        >
                            <el-input
                                    ...
                                    :formatter="(value) =>  { return verifyNumberCommaNo(value?.toString() || '0.00')}"
                            >
                                <template #prepend>
                                    ￥
                                </template>
            ...
            <template #footer>
				...
				<span class="dialog-footer">
                    <el-text tag="b"
                             style="margin-right: 15px">赠送金额：{{
                            verifyNumberComma(dynamicValidateForm.givePrice?.toString() || '0.00')
                        }}</el-text>
                    <el-text tag="b"
                             style="margin-right: 15px">本金金额：{{
                            verifyNumberComma(dynamicValidateForm.principalPrice?.toString() || '0.00')
                        }}</el-text>
                    <el-text tag="b"
                             style="margin-right: 15px">入账金额：{{
                            verifyNumberComma(dynamicValidateForm.occurPrice?.toString() || '0.00')
                        }}</el-text>
...
<script lang="ts" setup name="rechargeAccount">
    ...
    //确认
    const submit = async () => {
        if (dynamicValidateForm.principalPrice == 0.00 || dynamicValidateForm.principalPrice == '') {
