<template>
	<div class="inHospital-Cages" :class="isTagsViewCurrenFull?'tab-cus-full-Content':'tab-cus-Content'">
		<el-card shadow="never" :body-style="{ paddingBottom: '0' }">
			<el-form :model="queryParams" ref="queryForm" :inline="true">
				<el-form-item label="笼位名称">
					<el-input v-model="queryParams.name" clearable placeholder="请输入笼位名称" />
				</el-form-item>
				<el-form-item label="笼位类型">
		...
		<el-card shadow="never" class="full-table" style="overflow: auto; margin-top: 8px">
			<el-table :data="tableData"  v-loading="loading" tooltip-effect="light" row-key="id" border>
				...
				<el-table-column label="笼位类型" align="center" width="120">
			...
			/>
			<editDialog ref="editDialogRef" :title="editInHospitalCagesTitle" @reloadTable="handleQuery" />
		</el-card>
	</div>
</template>

<script lang="ts" setup name="inHospitalCages">
import { ref } from 'vue';
...
const editInHospitalCagesTitle = ref('');
...
const handleQuery = async () => {
	...
	var res = await getAPI(InHospitalApi).apiInHospitalGetRoomsPagePost(Object.assign(queryParams.value, tableParams.value));
...
// 打开新增页面
const openAddInHospitalCages = () => {
	editInHospitalCagesTitle.value = '添加笼位';
	editDialogRef.value.openDialog({ inHospitalCostType: 0, inHospitalType: 0, salePrice: 0, memberPrice: 0, monthSalePrice: 0, monthMemberPrice: 0, isEnable: true,capacity:1 });
};

// 打开编辑页面
const openEditInHospitalCages = (row: any) => {
	editInHospitalCagesTitle.value = '编辑笼位';
	editDialogRef.value.openDialog({ ...row });
};

// 删除
const delInHospitalCages = (row: any) => {
	ElMessageBox.confirm(`确定要删除吗?`, '提示', {
		...
		.then(async () => {
			await getAPI(InHospitalApi).apiInHospitalDeleteRoomsPost(row);
			handleQuery();
