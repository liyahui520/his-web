<template>
    <Table ref="tableMicroscopesRef" v-bind="tb.tableData" :getData="getData" border @sortHeader="onSortHeader"
        ...
        <template #action="scope" v-if="auth('products:microscopes:update') || auth('products:microscopes:delete')">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProductMicroscopes(scope.row)"
                v-auth="'products:microscopes:update'"> 编辑
            </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="danger" @click="delProductMicroscopes(scope.row)"
                v-auth="'products:microscopes:delete'"> 删除
            </el-button>
        </template>
    </Table>

    <editDialog ref="editDialogRef" :title="editProductMicroscopesTitle" @reloadTable="handleQuery"
...
const tb = reactive<TableDemoState>({
    tableData: {
        ...
        config: {
            ...
            isDisabled:false,
            exportFileName: '显微镜信息', //导出报表的文件名，不填写取应用名称
            // hideLoad: false,
            hidePrint: true
        },
...
const getData = async (par: any) => {
    let res = await getAPI(ProductMicroscopeApi).apiProductMicroscopePagePost(par);
    return res.data;
}
...
const handleQuery = async (data: any) => {
    ...
    await nextTick(() => {
        tableMicroscopesRef?.value?.pageReset();
    });
    loading.value = false;
};

// 打开新增页面
const openAddProductMicroscopes = (data: any) => {
    editProductMicroscopesTitle.value = '添加显微镜';
...
// 打开编辑页面
const openEditProductMicroscopes = (row: any) => {
    editProductMicroscopesTitle.value = '编辑显微镜';
    row.categoryArr = row.categoryId == -1 ? [] : [row.categoryId]
    editDialogRef.value.openDialog({ ...row });
};

// 删除
const delProductMicroscopes = (row: any) => {
    ElMessageBox.confirm(`确定要删除吗?`, '提示', {
        ...
        .then(async () => {
            await getAPI(ProductMicroscopeApi).apiProductMicroscopeDeletePost(row);
            handleQuery({});
...
//批量编辑
const batchEdit = () => {
    const tableRows = tableMicroscopesRef.value.getSelectionRows();
...
const batchDelete=()=>{
        ...
        ElMessageBox.confirm(`确定要删除吗？共 ` + tableRows.length + ` 条数据`, '提示', {
            ...
            .then(async () => {
                await getAPI(ProductMicroscopeApi).apiProductMicroscopeDeleteByIdsDelete({ids: tableRows.map(m=>{return m.id;})}).then(_ => {
                    nextTick(() => {
                        tableMicroscopesRef?.value?.pageReset();
                    });
                    ElMessage.success('删除成功');
...
onMounted(async () => {
    // 监听布局配'置弹窗点击打开
    mittBus.on('microscopesQueryMthods', async (params) => {
        await handleQuery(params);
    });
    // 监听布局配'置弹窗点击打开
    mittBus.on('microscopesAddMthods', async (params:any) => {
        openAddProductMicroscopes(params);
    });
