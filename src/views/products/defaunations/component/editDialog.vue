<template>
	<div class="product-defaunations-container">
		<el-dialog v-model="isShowDialog" :title="props.title" width="50%" draggable :close-on-click-modal="false">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" size="default" label-width="100px" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="间隔单位" prop="intervalUnit" class="input-interval">
							<el-select v-model="ruleForm.intervalUnit" filterable style="width: 100%">
...
const rules = ref<FormRules>({
	...
	interval: [{ required: true, message: '请输入接种间隔！', trigger: 'blur' }],
	intervalUnit: [{ required: true, message: '请选择接种间隔单位！', trigger: 'blur' }],
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id != undefined && ruleForm.value.id > 0) {
				await getAPI(ProductDefaunationsApi).apiProductDefaunationsEditPut(values);
			} else {
				await getAPI(ProductDefaunationsApi).apiProductDefaunationsAddPost(values);
			}
			closeDialog();
		}
	});
...
.product-defaunations-container {
	.input-interval .el-input {
		width: 30%;
	}

	.input-interval .el-select {
		width: 30%;
	}
