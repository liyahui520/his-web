<template>
	<div style="overflow: hidden;"
		:class="isTagsViewCurrenFull ? 'documentTemplate-container tab-cus-full-Content' : 'documentTemplate-container tab-cus-Content'">
		<el-container style="overflow: inherit">
			<el-aside class="sidebar" :width="sidebarWidth">
				<div class="sidebar-main" style="height: 100%" :width="sidebarWidth">
					<el-card style="height: 100%; transition: width 0.5s" :width="sidebarWidth"
						:body-style="{ padding: '0px', width: sidebarWidth, transition: 'width 0.5s' }">
						<el-scrollbar height="100%" :min-size="10">
							<div
								style="background-color: var(--el-color-primary); height: 40px; border-radius: 3px 3px 0 0">
								<p
									style="line-height: 40px; color: var(--el-color-white); font-size: 16px; font-weight: 800; margin-left: 20px; float: left">
									I</p>
								<p
									style="line-height: 40px; color: var(--el-color-white); font-size: 14px; font-weight: 500; margin-left: 10px; float: left">
									文书管理</p>
								<el-button
									style="float: right; margin-top: 8px; margin-right: 10px; border: var(--el-color-white) 1px solid"
									@click="add" size="small" type="primary">新增 </el-button>
							</div>
							<div class="prescript" v-loading="loading">
								<div style="display: grid; grid-template-columns: 170px auto" v-for="item in tableData"
									:key="item.id"
									:class="[activeItem == item.id ? 'active-prescript' : '', 'prescript-list']"
									@click="rowClick(item)">
									<span>{{ item.name }}</span>
									<span style="display: grid; grid-template-columns: 60px auto">
										<el-switch v-model="item.isEnable" class="ml-2" size="small" inline-prompt
											style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949"
											active-text="启用" inactive-text="停用" @change="update(item)" />
										<span><el-button type="danger" style="font-size: 12px; padding: 0px" link
												@click="deleteRow(item)">删除</el-button>
											<el-button type="primary" style="font-size: 12px; padding: 0px" link
												@click="renameRow(item)">重命名</el-button></span>
									</span>
								</div>
							</div>
						</el-scrollbar>
					</el-card>
					<el-button @click="toggleSidebar(isSidebarVisible)"
						:icon="isSidebarVisible ? DArrowLeft : DArrowRight" class="side-left"></el-button>
				</div>
			</el-aside>
			<el-container style="height: 100%">
				<el-main style="padding: 0; height: 100%">
					<el-card style="height: 100%" class="box-card" :body-style="{ padding: '0px' }" v-loading="loading">
						...
						<Editor ref="editorRef" :height="'calc(100%)'" :placeholder="''"
							@update:getText="updateAllergyHistoryText" :content="from.template ?? ''" :tempList="items"
							@update:getHtml="updateAllergyHistoryHtml" :toolbarConfig="toolbarConfig"
							:insert-node="insertNode" />
					</el-card>
				</el-main>
			...
			<el-aside class="sidebar-right" :width="sidebarRightWidth">
				<div class="sidebar-right-main" style="height: 100%" :width="sidebarRightWidth">
					<el-card style="height: 100%; transition: width 0.5s" :width="sidebarRightWidth"
						:body-style="{ padding: '0px', width: sidebarRightWidth, transition: 'width 0.5s' }">
						...
						<div style="padding: 10px">
							<el-form-item>
								<el-check-tag v-for="item in items" checked style="margin-right: 8px; margin-top: 8px"
									@change="
										(status: boolean) => {
											onChange(item);
										}
									" :key="item.label">{{ item.label }}</el-check-tag>
							</el-form-item>
						</div>
					</el-card>
					<el-button @click="toggleSidebarRight(isSidebarRightVisible)"
						:icon="isSidebarRightVisible ? DArrowRight : DArrowLeft" class="side-right"></el-button>
				</div>
			</el-aside>
		</el-container>
